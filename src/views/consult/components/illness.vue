...
import { uploadFileAPI } from '@/services/public'
...
const onDeleteImg = (item: UploaderFileListItem) => {
  form.value.pictures = form.value.pictures?.filter((i) => i.url !== item.url)
}
...
const disabled = computed(() => {
  return Object.values(form.value).some((i) => {
    if (i === undefined || i === '') return true
  })
})
...
onMounted(() => {
  ...
  if (illnessDesc) {
    showConfirmDialog({
      ...
      closeOnPopstate: false // 页面回退时对话框不关闭
    }).then(() => {
  ...
  <div class="consult-illness-page">
    ...
    <div class="illness-tip van-hairline--bottom">
      ...
      <div class="info">
        ...
        <p class="safe"><svg-icon name="consult-safe" /><span>内容仅医生可见</span></p>
      </div>
    </div>
    <!-- 收集信息 -->
